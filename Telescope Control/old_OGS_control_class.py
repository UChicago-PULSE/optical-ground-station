...
class OGS_control:
    ...
    def get_RA_DEC_degrees(self):
        ...
        RA,DEC=360*self.hex_to_dec(RA_s)/rot , 360*self.hex_to_dec(DEC_s)/rot
        return( RA,DEC)
    def polaris_align(self):
        '''
        The way this command is meant to be used is that the telescope should be pointing roughly at polaris. At
        ...
        full_rot=65536 
        RA_frac=RA_pol/360
        DEC_frac=DEC_pol/360
        RA_out=hex(round(full_rot*RA_frac))
        DEC_out=hex(round(full_rot*DEC_frac))
        print(RA_pol,DEC_pol)
        out_bstring=self.nexstarComm_read_until(nexstar.polaris_align_RA_DEC(RA_out,DEC_out) ,b"#")
        #time.sleep(3)
    ...
    def measure_runtime_of_sending_slew_rates(self):
        ...
        plt.show()
        return(np.average(time_list))
    def measured_wait(self,wait_time):
        print(f"Starting measured wait at {time.time()-self.global_start_time}")
        start_time=time.time()
        azm_alt_list=list()
        while (wait_time)-(time.time()-start_time)>=self.get_azm_alt_runtime:
            (azm_s,alt_z)=self.get_azmalt_degrees()
            dt=time.time()-self.global_start_time
            azm_alt_list.append([dt,azm_s,alt_z,self.slew_rates[0],self.slew_rates[1]])
            time.sleep(self.data_interval-self.get_azm_alt_runtime-0.0065)
        return(azm_alt_list)
    ...
    def follow_schedule(self,schedule):
        ...
        for time_ind in np.arange(len(schedule["time_array"])):
            ...
            m_azm_alt_list+=self.measured_wait(schedule["time_step"]-0.085)
        return(m_azm_alt_list,schedule)
    def follow_schedule_alt(self,schedule):
        self.global_start_time=time.time()
        """
        Takes in a schedule dict as built up by the Schedule_gen class and follows said schedule, doing measured waits at the time intervals
        and returning the recorded positions.
    ...
    def do_schedule_and_plot(self,schedule):
        data_list,schedule=self.follow_schedule_alt(schedule)
        self.Stop()
...
#control_object.polaris_align()
#print(control_object.get_azmalt_degrees())
control_object.horizon_north_align(alt_offset=alt_offset_val)
#control_object.do_and_plot()
#control_object.measure_runtime_of_sending_slew_rates()
#print(control_object.get_azmalt_degrees())
#control_object.Stop()
#time.sleep(1)
#control_object.Go_to_AZM_ALT(0,0)
"""
Makes a test pass
